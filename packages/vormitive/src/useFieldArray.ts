...
export const useFieldArray = <TEachValue>(fieldName: Ref<string>) => {
  ...
  const moveRight = (index: number) => {
    if (index !== elements.value.length - 1) {
      ...
      console.error(
        `Attempted to move element at index ${index} to the right, but it is the last element in the array`
      );
    }
  };
  ...
  const moveLeft = (index: number) => {
    if (index > 0) {
      ...
      console.error(
        `Attempted to move element at index ${index} to the left, but it is the first element in the array`
      );
    }
  };
